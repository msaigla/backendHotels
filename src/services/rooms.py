...
from src.exceptions import (ObjectNotFoundException,
                            RoomNotFoundException, check_date_to_after_date_from)
...
class RoomService(BaseService):
    ...
    async def create_room(self, hotel_id: int, room_data: RoomAddRequest):
        ...
        data = await self.db.rooms.add(_room_data)
    ...
    async def edit_room(self, hotel_id: int, room_id: int, room_data: RoomAddRequest):
        ...
        _room_data = RoomAdd(hotel_id=hotel_id, **room_data.model_dump())
        self.db.rooms.edit(data=_room_data, id=room_id)
        await self.db.rooms_facilities.set_room_facilities(
            room_id=room_id, facilities_ids=room_data.facilities_ids
        )
        await self.db.commit()

    async def edit_room_patch(self, hotel_id: int, room_id: int, room_data: RoomPatchRequest):
        await HotelService(self.db).get_hotel_with_check(hotel_id)
        await self.get_room_with_check(room_id)
        _room_data_dict = room_data.model_dump(exclude_unset=True)
        _room_data = RoomPatch(hotel_id=hotel_id, **_room_data_dict)
        self.db.rooms.edit(data=_room_data, exclude_unset=True, id=room_id, hotel_id=hotel_id)
